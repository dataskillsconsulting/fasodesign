...
export type SearchBoxProps = { options: SearchOption[]; onSelect: (option: SearchOption) => void; label?: string; placeholder?: string; emptyText?: string };
export function SearchBox({ options, onSelect, label = "Rechercher", placeholder = "Rechercher…", emptyText = "Aucun résultat" }: SearchBoxProps) {
  const [query, setQuery] = useState(""); const [active, setActive] = useState(0); const [open, setOpen] = useState(false); const listId = useId(); const statusId = useId(); const rootRef = useRef<HTMLDivElement>(null);
  ...
  function choose(index: number) { const option = results[index]; if (option) { onSelect(option); setQuery(option.label); setOpen(false); } }
  function clear() { setQuery(""); setOpen(false); setActive(0); }
  return <div className="command-search" ref={rootRef}><div><Search aria-hidden="true" /><input role="combobox" aria-label={label} aria-expanded={open} aria-controls={open ? listId : undefined} aria-describedby={open ? statusId : undefined} aria-autocomplete="list" aria-activedescendant={open && results[active] ? `${listId}-${active}` : undefined} value={query} placeholder={placeholder} onFocus={() => { if (query) setOpen(true); }} onChange={(event) => { setQuery(event.target.value); setActive(0); setOpen(Boolean(event.target.value)); }} onKeyDown={(event) => { if (event.key === "ArrowDown") { event.preventDefault(); setOpen(true); setActive((value) => Math.min(value + 1, Math.max(0, results.length - 1))); } else if (event.key === "ArrowUp") { event.preventDefault(); setOpen(true); setActive((value) => Math.max(value - 1, 0)); } else if (event.key === "Enter" && open) { event.preventDefault(); choose(active); } else if (event.key === "Escape") { event.preventDefault(); setOpen(false); } }} />{query ? <button type="button" onClick={clear} aria-label="Effacer la recherche"><X /></button> : null}</div><span id={statusId} className="sr-only" role="status" aria-live="polite">{open ? `${results.length} résultat${results.length > 1 ? "s" : ""}` : ""}</span>{open ? <div id={listId} role="listbox" aria-label={`Résultats pour ${query}`}>{results.length ? results.map((option, index) => <button type="button" role="option" aria-selected={index === active} id={`${listId}-${index}`} key={option.value} onMouseEnter={() => setActive(index)} onClick={() => choose(index)}><strong>{option.label}</strong>{option.description ? <small>{option.description}</small> : null}</button>) : <p>{emptyText}</p>}</div> : null}</div>;
}
